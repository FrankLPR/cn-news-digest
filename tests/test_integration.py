...
from tests.conftest import rss_date, cst_time_str


# ---- Mock responses for each source (dynamic dates) ----

def _wallstreetcn_rss():
...
async def test_three_sources_produce_articles():
    ...
    respx.get("https://rsshub.app/wallstreetcn/hot/day").mock(
        return_value=httpx.Response(200, text=_wallstreetcn_rss())
    )
    # Sina via RSSHub
    respx.get("https://rsshub.app/sina/finance/rollnews/2509").mock(
        return_value=httpx.Response(200, text=_sina_rss())
    )
    ...
    respx.get("https://i.news.qq.com/gw/event/pc_hot_ranking_list").mock(
        return_value=httpx.Response(200, json=_tencent_api_response())
    )
...
async def test_four_sources_with_xueqiu_playwright():
    ...
    mock_response.text = AsyncMock(
        return_value='{"list": ' + json.dumps(mock_hot_events) + "}"
    )
